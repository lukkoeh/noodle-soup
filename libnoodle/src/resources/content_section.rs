...
pub mod http {
    use super::*;
    use axum::{Json, extract::{Path as UrlPath, State}, http::StatusCode, response::{IntoResponse, Response}};

    /* -------- Sections ---------- */

    //GET /course/{courseId}/sections
    pub async fn get_sections(UrlPath(course_id): UrlPath<i64>, State(state): State<crate::AppState>) -> Response {
    ...
    pub async fn create_section(
        ...
        match sqlx::query_scalar::<_, i64>("INSERT INTO content_section(course_id, headline, order_index) VALUES ($1,$2,$3) RETURNING uid")
            ...
            .await
        {
            Ok(uid) => Json(ContentSection { section_id: uid, course_id, headline: req.headline, order_index: idx }).into_response(),
            Err(_) => StatusCode::INTERNAL_SERVER_ERROR.into_response(),
        }
    }
    ...
    pub async fn update_section(
        ...
        {
            Ok(res) => {
                if res.rows_affected()==0 { StatusCode::NOT_FOUND.into_response() } else { Json(ContentSection{section_id,course_id,headline:req.headline,order_index:idx}).into_response() }
            }
            Err(_) => StatusCode::INTERNAL_SERVER_ERROR.into_response(),
        }
    }
    ...
    pub async fn create_content(
        ...
        // verify section
        match sqlx::query_scalar::<_, i32>("SELECT 1 FROM content_section WHERE uid=$1 AND course_id=$2")
            .bind(section_id)
            .bind(course_id)
            .fetch_optional(&state.db)
            .await
    ...
    pub async fn update_content(
        ...
        match sqlx::query(query)
            ...
            .await
        {
            Ok(r) => if r.rows_affected()==0 { StatusCode::NOT_FOUND.into_response() } else { Json(elem).into_response() },
            Err(_) => StatusCode::INTERNAL_SERVER_ERROR.into_response(),
        }
    }
